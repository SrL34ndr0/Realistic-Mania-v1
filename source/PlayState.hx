...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (SONG == null)
			SONG = Song.loadFromJson('madriza');
		...
		{
			case 'achul': //Week 1
			var bgMamon:FlxSprite = new FlxSprite().loadGraphic(Paths.image('achul'));
			add(bgMamon);
			case 'rocho': //Week roca
			var roca:FlxSprite = new FlxSprite().loadGraphic(Paths.image('unknown'));
			roca.scale.set(0.9, 0.9);
			add(roca);
			case 'batr': //secret
			var waza:FlxSprite = new FlxSprite().loadGraphic(Paths.image('waza'));
			add(waza);
		}
